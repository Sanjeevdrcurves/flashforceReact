...
useEffect(()=>{
  ...
  const handleSaveCategory = async () => {
    ...
    try {
      ...
      if (editingCategoryId !== null) {
        // Update category API
        response = await fetch(`${API_URL}/PlanCategory/Update?id=${editingCategoryId}&planCategoryName=${categoryName}&modifiedBy=system`, {
          method: 'POST',
  ...
  const handleDeleteCategory = async (id) => {
    try {
      setLoading(true);
      const response = await fetch(`${API_URL}/PlanCategory/${id}?modifiedBy=system`, {
        method: 'DELETE',
      });
      ...
      <div className="toolbar mb-4 flex flex-col sm:flex-row gap-2 items-center" >
        ...
        <div className="flex gap-2 items-center">
          ...
          <button
            ...
            className={`btn ${editingCategoryId !== null ? 'btn-success' : 'btn-primary'}`}
            disabled={loading}
          >

...
const EditPlace = () => {
  ...
  const { placeId, name, objecttypeid, objectid } = useParams() || { placeId: 1001 };
  const [objectTypes, setObjectTypes] = useState([]);
  const [selectedType, setSelectedType] = useState(null);
  const [formConfig, setFormConfig] = useState(null);
  const [serverPlaceRecord, setServerPlaceRecord] = useState(null);
  ...
  useEffect(() => {
    const fetchObjectTypes = async () => {
      try {
        const response = await axios.get(`${API_URL}/CustomObject/GetObjectDetails?objectId=26`);
  ...
  useEffect(() => {
    const fetchPlace = async () => {
      try {
        const url = `${API_URL}/CustomObject/GetPlaces?placeId=${placeId}&companyId=${companyId}&customObjectTypeId=0`;
        const response = await axios.get(url);
        const record = response.data[0];
        console.log('ServerPlaceRecord:', record);
        setServerPlaceRecord(record);
    ...
    fetchPlace();
  }, [placeId, companyId, name, objecttypeid, objectid]);
  ...
  useEffect(() => {
    if (!formConfig || !serverPlaceRecord) return;
    setInitialValues((prevValues) => {
      // Only build once if prevValues is empty
      if (Object.keys(prevValues).length > 0) return prevValues;

      let defaultFieldArray = [];
      if (serverPlaceRecord.CustomFieldData) {
        try {
          defaultFieldArray = JSON.parse(serverPlaceRecord.CustomFieldData);
        } catch (err) {
        ...
        formConfig.fields.forEach((section) => {
          if (Array.isArray(section.fields)) {
            section.fields.forEach((field) => {
              if (field.source === 'core' && field.dbcoloumName) {
                let value = serverPlaceRecord[field.dbcoloumName];
  ...
  const handleFormSubmit = async (values) => {
    ...
    try {
      ...
      const updatedRecord = {
        PlaceId: serverPlaceRecord.PlaceId,
        ...updatedCoreProps,
      ...
      const payload = {
        ...
        activityId: serverPlaceRecord.PlaceId,
        activityData: JSON.stringify(updatedRecord),
      };

      console.log('Submitting payload:', payload);
      const response = await axios.post(`${API_URL}/CustomObject/ManagePlace`, payload);
  ...
  return (
    <div>
      ...
      <div className="container-fixed">
        ...
        <pre>ServerPlaceRecord: {JSON.stringify(serverPlaceRecord, null, 2)}</pre>

...
const CustomFieldPersonResponseDemo = () => {
  // ====================
  // Data & Navigation
  // ====================
    ...
    const fetchObjectTypes = async () => {
      try {
        const response = await axios.get(
          `${API_URL}/CustomObject/GetObjectDetails?objectId=22`
        );
    ...
    const fetchRows = async () => {
      try {
        ...
        const res = await axios.get(
          `${API_URL}/CustomObject/GetPersons?personId=${personId}&companyId=${compId}&customObjectTypeId=${customObjectTypeId}`
        );
  ...
  const handleEdit = (row) => {
    ...
    //alert(row);
    navigate(
      `/Person/EditPerson/${selectedType.ObjectTypeName}/${selectedType.ObjectID}/${row.CustomObjectTypeId}/${row.PersonID}`
    );
  };
  ...
  return (
    <Box>
      <MaterialReactTable
        getRowId={(row) => row.id || row.PersonId || Math.random()}
...
export default CustomFieldPersonResponseDemo;

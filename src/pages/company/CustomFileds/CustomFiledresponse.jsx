...
const CustomFieldResponseDemo = () => {
    ...
    const fetchObjectTypes = async () => {
      try {
        const response = await axios.get(
          `${API_URL}/CustomObject/GetObjectDetails?objectId=15`
        );
    ...
    const fetchRows = async () => {
      try {
        ...
        const res = await axios.get(
          `${API_URL}/CustomObject/GetActivities?activityId=${activityId}&companyId=${compId}&customObjectTypeId=${customObjectTypeId}`
        );
  ...
  const handleEdit = (row) => {
    ...
    navigate(
      `/company/editactivity/${selectedType.ObjectTypeName}/${selectedType.ObjectID}/${row.CustomObjectTypeId}/${row.ActivityId}`
    );
  };
...
export default CustomFieldResponseDemo;

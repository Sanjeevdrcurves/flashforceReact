...
import EmailModal from "./block/EmailModal";

const API_URL = import.meta.env.VITE_FLASHFORCE_API_URL;

const CustomFieldCommData = () => {
  // ====================
  // Data & Navigation
  // ====================
    ...
    const fetchObjectTypes = async () => {
      try {
        const response = await axios.get(
          `${API_URL}/CustomObject/GetObjectDetails?objectId=23`
        );
    ...
    const fetchRows = async () => {
      try {
        ...
        const res = await axios.get(
          `${API_URL}/CustomObject/GetComms?commId=${commId}&companyId=${compId}&customObjectTypeId=${customObjectTypeId}`
        );
        // Keep the entire array of rows; copy 'JsonData'
  ...
  const handleEdit = (row) => {
    ...
    navigate(
      `/company/editcomm/${selectedType.ObjectTypeName}/${selectedType.ObjectID}/${row.CustomObjectTypeId}/${row.CommId}`
    );
  };
   ...
   * ignoring JsonData. For columns with source !== 'core', parse JsonData.
   */
    ...
    const columns = dynamicColumns.map((col) => ({
      ...
      header:
        saveColumnSwitches[col.id] && layoutName
          ...
          : col.label,
      accessorFn: (row) => {
        if(col.dbColumnName =="Body" || col.dbColumnName =="JsonData"){
          return "-";
        }
        if (col.source === "core") {
          if (col.dbColumnName) {
          ...
          let parsedData;
          try {
            parsedData = JSON.parse(row.jsonData);
          } catch (error) {
    ...
    columns.push({
      ...
      Cell: ({ row }) => (
        <Box sx={{ display: "flex", gap: "16px" }}>
          ...
          <Button variant="outlined" onClick={() => handleDetailsModalOpen(row.original)}>
            Details
          </Button>
        </Box>
      ),
  ...
  return (
    ...
    <Box>
      <MaterialReactTable
        getRowId={(row) => row.id || row.CommId || Math.random()}
    ...
    {selectedEmail && (
      <div>
        <EmailModal open={detailsModalOpen} onOpenChange={handleDetailsModalClose} selectedEmail={selectedEmail}/>
      </div>
    )}
    </div>
  );
...
export default CustomFieldCommData;

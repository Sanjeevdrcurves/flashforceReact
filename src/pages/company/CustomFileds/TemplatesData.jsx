...
const TemplatesData = () => {
    ...
    const fetchObjectTypes = async () => {
      try {
        const response = await axios.get(
          `${API_URL}/CustomObject/GetObjectDetails?objectId=28`
        );
    ...
    const fetchRows = async () => {
      try {
        ...
        const res = await axios.get(
          `${API_URL}/CustomObject/GetTemplates?templateId=${templateId}&companyId=${compId}&customObjectTypeId=${customObjectTypeId}`
        );
  ...
  const handleEdit = (row) => {
    ...
    navigate(
      `/template/EditTemplates/${selectedType.ObjectTypeName}/${selectedType.ObjectID}/${row.CustomObjectTypeId}/${row.TemplateID}`
    );
  };

  const handleDelete = (rowId) => {
    if (window.confirm("Are you sure you want to delete this row?")) {
      setRows((prev) => prev.filter((r) => r.TemplateID!== rowId));
    ...
    columns.push({
      ...
      Cell: ({ row }) => (
        <Box sx={{ display: "flex", gap: "16px" }}>
          ...
          <Button variant="contained" color="error" onClick={() => handleDelete(row.original.TemplateId)}>
            Delete
          </Button>
        </Box>
      ),
  ...
  return (
    <Box>
      <MaterialReactTable
        getRowId={(row) => row.id || row.ActivityId || Math.random()}

...
const NotesData = () => {
    ...
    const fetchObjectTypes = async () => {
      try {
        const response = await axios.get(
          `${API_URL}/CustomObject/GetObjectDetails?objectId=27`
        );
    ...
    const fetchRows = async () => {
      try {
        ...
        const res = await axios.get(
          `${API_URL}/CustomObject/GetNotes?noteId=${noteId}&companyId=${compId}&customObjectTypeId=${customObjectTypeId}`
        );
  ...
  const handleEdit = (row) => {
    ...
    navigate(
      `/Notes/EditNotes/${selectedType.ObjectTypeName}/${selectedType.ObjectID}/${row.CustomObjectTypeId}/${row.NoteId}`
    );
  };

  const handleDelete = (rowId) => {
    if (window.confirm("Are you sure you want to delete this row?")) {
      setRows((prev) => prev.filter((r) => r.NoteId !== rowId));
    ...
    columns.push({
      ...
      Cell: ({ row }) => (
        <Box sx={{ display: "flex", gap: "16px" }}>
          ...
          <Button variant="contained" color="error" onClick={() => handleDelete(row.original.NoteId)}>
            Delete
          </Button>
        </Box>
      ),

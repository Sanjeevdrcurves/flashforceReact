...
const PlatformCount = ({platformCount}) => {
  const items = [{
    logo: 'linkedin-2.svg',
    info: '9.3k',
    desc: 'Amazing mates',
  ...
  const renderItem = (item, index) => {
    return <div key={index} className="card flex-col  gap-6 h-full bg-cover rtl:bg-[left_top_-1.7rem] bg-[right_top_-1.7rem] bg-no-repeat channel-stats-bg">
        {item.logoDark ? <>
            ...
            <img src={toAbsoluteUrl(`/media/brand-logos/${item.logo}`)} className="dark:hidden w-7 mt-4 ms-5" alt="" />
            <img src={toAbsoluteUrl(`/media/brand-logos/${item.logoDark}`)} className="light:hidden w-7 mt-4 ms-5" alt="" />
          </> : <i className={`ki-duotone text-3xl gap-1 px-5 ${item.logo}`}></i>
          }

...
const EditPerson = () => {
  ...
  const { personId, name, objecttypeid, objectid } = useParams() || { personId: 1001 };
  ...
  useEffect(() => {
    const fetchObjectTypes = async () => {
      try {
        const response = await axios.get(`${API_URL}/CustomObject/GetObjectDetails?objectId=22`);
  ...
  useEffect(() => {
    const fetchActivity = async () => {
      try {
        const url = `${API_URL}/CustomObject/GetPersons?personId=${personId}&companyId=${companyId}&customObjectTypeId=0`;
    ...
    fetchActivity();
  }, [personId, companyId, name, objecttypeid, objectid]);
  ...
  const handleFormSubmit = async (values) => {
    ...
    try {
      ...
      const updatedRecord = {
        PersonID: serverActivityRecord.PersonID,
        ...updatedCoreProps,
      ...
      const payload = {
        ...
        ActivityId: serverActivityRecord.PersonID,
        ActivityData: JSON.stringify(updatedRecord),
      };

      console.log('Submitting payload:', payload);
      const response = await axios.post(`${API_URL}/CustomObject/ManagePerson`, payload);

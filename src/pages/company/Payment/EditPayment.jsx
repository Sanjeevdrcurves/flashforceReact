...
const EditPayment = () => {
  ...
  const { paymentId, name, objecttypeid, objectid } = useParams() || { paymentId: 1001 };
  debugger
  ...
  useEffect(() => {
    const fetchActivity = async () => {
      try {
        const url = `${API_URL}/CustomObject/GetPayment?paymentId=${paymentId}&companyId=${companyId}&customObjectTypeId=0`;
    ...
    fetchActivity();
  }, [paymentId, companyId, name, objecttypeid, objectid]);
  ...
  const handleFormSubmit = async (values) => {
    ...
    try {
      ...
      const updatedRecord = {
        paymentId: serverActivityRecord.paymentId,
        ...updatedCoreProps,

...
const PaymentData = () => {
    ...
    const fetchObjectTypes = async () => {
      try {
        const response = await axios.get(
          `${API_URL}/CustomObject/GetObjectDetails?objectId=30`
        );
    ...
    const fetchRows = async () => {
      try {
        ...
        const res = await axios.get(
          `${API_URL}/CustomObject/GetPayment?paymentId=${PaymentId}&companyId=${compId}&customObjectTypeId=${customObjectTypeId}`
        );
  ...
  const handleEdit = (row) => {
    ...
    //alert(row);
    navigate(
      `/Payment/EditPayment/${selectedType.ObjectTypeName}/${selectedType.ObjectID}/${row.CustomObjectTypeId}/${row.PaymentId}`
    );
  };
  ...
  const handleExportRowsCSV = useCallback(
    (rowsToExport, table) => {
      ...
      const link = Payment.createElement("a");
      link.href = url;
      link.setAttribute("download", "export.csv");
      Payment.body.appendChild(link);
      link.click();
      Payment.body.removeChild(link);
  ...
  return (
    <Box>
      <MaterialReactTable
        getRowId={(row) => row.id || row.PaymentId || Math.random()}

...
const Payment = () => {
  ...
  useEffect(() => {
    const fetchObjectTypes = async () => {
      try {
        const response = await axios.get(`${API_URL}/CustomObject/GetObjectDetails?objectId=30`);
  ...
  const handleFormSubmit = async (values) => {
    ...
    try {
      ...
      const payload = {
        operationType: "Add",
        PaymentData: JSON.stringify(coreArray[0]),
        companyId, // assume available from Redux or similar
        customObjectTypeId: selectedType.ObjectTypeID,
      };
  
      const response = await axios.post(`${API_URL}/CustomObject/ManagePayment`, payload);

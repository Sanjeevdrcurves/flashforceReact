...
const AddPipleline = () => {
  ...
  useEffect(() => {
    const fetchObjectTypes = async () => {
      try {
        const response = await axios.get(`${API_URL}/CustomObject/GetObjectDetails?objectId=21`);
  ...
  const handleFormSubmit = async (values) => {
    ...
    try {
      ...
      const coreArray = [
        {
          ...coreData,
          PipelineData: JSON.stringify(defaultFields),
        },
      ];
      ...
      const payload = {
        operationType: "Add",
        PipelineData: JSON.stringify(coreArray[0]),

...
const EditDocument = () => {
  ...
  const { DocumentId, name, objecttypeid, objectid } = useParams() || { DocumentId: 1001 };
  debugger
  const [objectTypes, setObjectTypes] = useState([]);
  const [selectedType, setSelectedType] = useState(null);
  const [formConfig, setFormConfig] = useState(null);
  const [serverDocumentRecord, setServerDocumentRecord] = useState(null);
  ...
  useEffect(() => {
    const fetchObjectTypes = async () => {
      try {
        const response = await axios.get(`${API_URL}/CustomObject/GetObjectDetails?objectId=29`);
  ...
  useEffect(() => {
    const fetchDocument = async () => {
      try {
        const url = `${API_URL}/CustomObject/GetActivities?DocumentId=${DocumentId}&companyId=${companyId}&customObjectTypeId=0`;
        const response = await axios.get(url);
        const record = response.data[0];
        console.log('ServerDocumentRecord:', record);
        setServerDocumentRecord(record);
    ...
    fetchDocument();
  }, [DocumentId, companyId, name, objecttypeid, objectid]);
  ...
  useEffect(() => {
    if (!formConfig || !serverDocumentRecord) return;
    setInitialValues((prevValues) => {
      // Only build once if prevValues is empty
      if (Object.keys(prevValues).length > 0) return prevValues;

      let defaultFieldArray = [];
      if (serverDocumentRecord.CustomFieldData) {
        try {
          defaultFieldArray = JSON.parse(serverDocumentRecord.CustomFieldData);
        } catch (err) {
        ...
        formConfig.fields.forEach((section) => {
          if (Array.isArray(section.fields)) {
            section.fields.forEach((field) => {
              if (field.source === 'core' && field.dbcoloumName) {
                let value = serverDocumentRecord[field.dbcoloumName];
  ...
  }, [formConfig, serverDocumentRecord]);
  ...
  const handleFormSubmit = async (values) => {
    ...
    try {
      ...
      const updatedRecord = {
        DocumentId: serverDocumentRecord.DocumentId,
        ...updatedCoreProps,
      ...
      const payload = {
        ...
        DocumentId: serverDocumentRecord.DocumentId,
        DocumentData: JSON.stringify(updatedRecord),
      };

      console.log('Submitting payload:', payload);
      const response = await axios.post(`${API_URL}/CustomObject/ManageDocument`, payload);
  ...
  return (
    <div>
      ...
      <div className="container-fixed">
        ...
        <pre>ServerDocumentRecord: {JSON.stringify(serverDocumentRecord, null, 2)}</pre>

...
const EditTemplates = () => {
  ...
  const { templateId, name, objecttypeid, objectid } = useParams() || { templateId: 1001 };
  ...
  useEffect(() => {
    const fetchObjectTypes = async () => {
      try {
        const response = await axios.get(`${API_URL}/CustomObject/GetObjectDetails?objectId=28`);
  ...
  useEffect(() => {
    const fetchActivity = async () => {
      try {
        const url = `${API_URL}/CustomObject/GetTemplates?templateId=${templateId}&companyId=${companyId}&customObjectTypeId=0`;
    ...
    fetchActivity();
  }, [templateId, companyId, name, objecttypeid, objectid]);
  ...
  useEffect(() => {
    ...
    setInitialValues((prevValues) => {
      ...
      if (serverActivityRecord.JsonData) {
        try {
          defaultFieldArray = JSON.parse(serverActivityRecord.JsonData);
        } catch (err) {
          console.error('Failed to parse JsonData:', err);
        }
      }
  ...
  const handleFormSubmit = async (values) => {
    ...
    try {
      ...
      const updatedRecord = {
        TemplateId: serverActivityRecord.TemplateID,
        ...updatedCoreProps,
        JsonData: defaultFields,
      };
      ...
      const payload = {
        ...
        templateId: serverActivityRecord.TemplateID,
        templateData: JSON.stringify(updatedRecord),
      };

      console.log('Submitting payload:', payload);
      const response = await axios.post(`${API_URL}/CustomObject/ManageTemplates`, payload);

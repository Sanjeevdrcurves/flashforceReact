...
const GoogleEmailList = () => {
  ...
  const fetchEmails = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${API_URL}/email/fetch-filtered`, {
        params: { pageNumber: page, pageSize, debouncedSearch, filter, userId, type:"GMAIL" },
      });
        ...
        {loading ? (
            ...
            <table className="w-full border-collapse border border-gray-200">
                ...
                <tr className="bg-gray-100">
                    ...
                    <th className="p-2 border border-gray-200 text-left">Sender</th>
                    <th className="p-2 border border-gray-200 text-left">Subject</th>
                    <th className="p-2 border border-gray-200 text-left">Received Date</th>
                </tr>
                </thead>
                <tbody>
                {emails.map((email) => (
                    ...
                    <td className="p-2 border border-gray-200">{email.senderName}</td>
                    <td className="p-2 border border-gray-200 truncate">{email.subject}</td>
        ...
        <div className="p-4 overflow-y-auto h-full">
          {selectedEmail ? (
            <div>
              ...
              <p className="text-gray-500 mb-2">{new Date(selectedEmail.receivedDateTime).toLocaleString()}</p>
...
export default GoogleEmailList;

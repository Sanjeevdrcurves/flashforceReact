...
import { DropdownUserEntities } from './DropdownUserEntities';
import { useSettings } from '@/providers/SettingsProvider';
import { DefaultTooltip, KeenIcon } from '@/components';
import { MenuItem, MenuLink, MenuSub, MenuTitle, MenuSeparator, MenuArrow, MenuIcon } from '@/components/menu';
import { AccountSettingsModal } from '../../../pages/account/home/settings-modal/AccountSettingsModal';
...
//   } = useAuthContext();
  ...
  const logout=()=>{
    ...
    distach(Logout());
    navigate('/auth/classic/login', { replace: true });
  ...
  const buildHeader = () => {
    ...
    return <div className="flex items-center justify-between px-5 py-1.5 gap-1.5">
        <div className="flex items-center gap-2">
          <img className="size-9 rounded-full border-2 border-success" src={toAbsoluteUrl(`/media/avatars/${user.imageName}`)} alt="" />
          <div className="flex flex-col gap-1.5">
            <Link to="/account/hoteme/get-stard" className="text-sm text-gray-800 hover:text-primary font-semibold leading-none">
              {user?.fullName}
            </Link>
            <a href="#" className="text-xs text-gray-600 hover:text-primary font-medium leading-none">
            {user?.email}
            </a>
          </div>
        </div>
  ...
  const buildMenu = () => {
    return <Fragment>
        ...
        <div className="flex flex-col">
          <DropdownUserEntities menuItemRef={menuItemRef} />
          <MenuSeparator />
          <MenuItem>
          ...
          <MenuItem>
            <MenuLink path="/public-profile/drcurves/UserProfile">
              <MenuIcon>
          ...
          <MenuItem  onClick={()=>{setsettingModal(true)}}>
            <MenuLink  >
              <MenuIcon>
              ...
              <MenuTitle>
                ...
                <FormattedMessage id="USER.MENU.COMPANYSETTING" />
              </MenuTitle>
            </MenuLink>
          </MenuItem>
